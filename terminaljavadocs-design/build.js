...
const PATHS = {
  ...
  pluginResourcesDir: join(
    __dirname,
    "..",
    "terminaljavadocs-maven-plugin",
    "src",
    "main",
    "resources",
    "styles",
...
const CSS_ENTRIES = [
  {
    entry: "main.css",
    output: "terminaljavadocs",
    expanded: true,
    plugin: false,
  },
  {
    entry: "page.css",
    output: "terminaljavadocs-site",
    expanded: false,
    plugin: true,
  },
  {
    entry: "landings.css",
    output: "terminaljavadocs-landing",
    expanded: false,
    plugin: true,
  },
  {
    entry: "javadocs.css",
    output: "terminaljavadocs-javadoc",
    expanded: false,
    plugin: true,
  },
  {
    entry: "jacoco.css",
    output: "terminaljavadocs-coverage",
    expanded: false,
    plugin: true,
  },
  {
    entry: "jxr.css",
    output: "terminaljavadocs-jxr",
    expanded: false,
    plugin: true,
  },
];
...
async function buildCSSEntry({ entry, output, expanded, plugin }) {
  ...
  if (plugin) {
    await writeFile(
      join(PATHS.pluginResourcesDir, `${output}.min.css`),
      minifiedResult.css,
    );
  }
  ...
  try {
    ...
    for (const result of cssResults) {
      ...
      if (result.sizes.expanded) {
        ...
        console.log(
          `  ${result.output}.min.css`.padEnd(38) +
            `${minSize} KB${pluginMarker}`,
        );
      } else {
        console.log(
          `  ${result.output}.min.css`.padEnd(38) +
    ...
    console.log(
      `  terminaljavadocs.min.js`.padEnd(38) + `${jsSize} KB → plugin`,
    );
    console.log(`  inject.min.js`.padEnd(38) + `${injectSize} KB`);

    console.log("\n✓ Copied to ../target/staging/");
    console.log(
      `✓ Copied ${pluginCssCount} CSS + 1 JS to maven plugin resources\n`,
    );
  } catch (err) {
